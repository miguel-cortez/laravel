...
A continuación presento el contenido predeterminado del archivo de configuración:  
...
:books: Nota. Para crear una copia de seguridad, puede dejar las configuraciones por defecto.  
...
Lo que se hace es modificar la sección `disks`, en la clave `root` del siguiente contenido:  
...
Si no se cambia, por defecto se guardan las copias de seguridad en el directorio `C:\wamp64\www\example_app\storage\app\private`. Aquí se crea automáticamente un directorio llamado `Laravel` y dentro encontrará los backups.  

## Configurar la ruta de la aplicación de consola que crea las copias de seguridad.  

Para `MySQL` se usará `mysqldump`, por lo tanto, se debe configurar en el archivo `config\database.php` 
...
```
:star: En el ejemplo anterior, se ha programado a nivel de demostración, crear un backup a cada minuto.  

Comando para iniciar la ejecución de las copias automáticas:  
...
En la imagen anterior se puede ver la programacón y la ejecución de dos copias de seguridad automáticas.

Luego, al ver la carpeta de los backups se tiene tres copias porque ya teníamos una que creamos anteriormente.  
